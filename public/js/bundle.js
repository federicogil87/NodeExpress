...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiZmVkZXJpY29naWw4NyIsImEiOiJja21rcHhzZWsxM3V5MnBsOG5kdzA0MWpqIn0.qetc_ZUPB0hiWVwjTxJJXw';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/federicogil87/ckmkq0bik7ltt17n4uc1hmxxn',
    scrollZoom: false // interactive: false,
    // zoom: 10,
    // center: [-0.1, 51.509],
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Add popup to locations

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Extemd map bounds to include current location
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', 'Logout Successful');
              window.setTimeout(function () {
                location.assign('/'); // location.reload(true);
              }, 1500);
            }
